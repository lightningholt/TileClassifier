...
def folder_management(save_str,pre_path='runs',append_date = True,overwrite = False):
    ...
    if os.path.exists(save_str):
        if overwrite:
            print('Warning: Folder '+str(save_str)+ ' exists already. Continuing')
        else:
            print('ERROR: run name already exists! Try another and start again!')
            sys.exit()
    else:
...
def is_pollock(x,group = ('P','J')):
    if isinstance(x,str):
        return x.startswith(group)
    return x.name.startswith(group)

# def is_pollock(x,group = ('D','E')):
#     if isinstance(x,str):
#         return x.startswith(group)
#     return x.name.startswith(group)
...
def get_dls(M,valid,path,folders,hold_out_files=[],px_size = 256,grayscale=False,flipped = True,multipath = False,selected_group = ('P','J')):
    ...
    # def is_group(x,group = selected_group):
    #     return is_pollock(x,group=group)
    ...
    dls = paintblock.dataloaders(path,num_workers = 0)
    # dls.show_batch()
    return dls
...
'P47(F Lowres)','P65(L)','P75(V)','P77(J)','P80(J)','P86(S)','P105(J)','P106(V)','P115(F Lowres)','A14(Left)'],
    ...
    if isinstance(valid_set,float):
        ...
        while (np.abs(ratio_df['ratio']) > 0.05).any():
            ...
            valid_ratio = find_dist_of_pollocks_imitations(valid, master, TRAIN=False,groups = pollock_groups)
            train_ratio = find_dist_of_pollocks_imitations(valid, master, TRAIN=True,groups = pollock_groups)
    ...
    # arch = timm.create_model('levit_128', pretrained=False, num_classes=dls.c) # inserted
    ...
    # learn = Learner(dls, arch, metrics=error_rate) #changed to arch from previous line
...
def model_run2(master, save_str, 
              ...
              thread_num = False,
              computer = '',
              px_size = 224, #this seems to be important when running through the Learner() function which I think I need for timm.create_model type models
              grayscale = False,
              epochs = 1,
              arch = resnet50,
              folders = ['10','100','105', '110', '115', '120', '125',
...
'P47(F Lowres)','P65(L)','P75(V)','P77(J)','P80(J)','P86(S)','P105(J)','P106(V)','P115(F Lowres)','A14(Left)'],
    ...
    # print(set([item.stem[0] for item in dls.train.items])) 
    # print(set([item.stem[0] for item in dls.valid.items]))
    arch = timm.create_model(arch, pretrained=pretrained, num_classes=dls.c) # inserted
...
def model_analysis(dls,preds,save_path = False,prob_cutoff = 0.5,runtime = 0,date = '',folders = [],Descreened = [], notes = '',write_name = 'metrics.csv',valid_set = '',seed = '',px_size = '',grayscale = '',epochs = '',hold_out_catalog = [],hold_out_files=[], hold_out_duplicates = [],hold_out_files_full= [],computer = '',failed_column = 'sizesF_binaryT_failed',model='',multipath = ''):
    # print('dls',dls)
...
def get_results_df(dls,preds,save_path = False,write_name = 'results.csv', TRAIN=False):
    ...
    def return_prediction(num,prob=0.5):
        if num > prob:
            return 1
        else:
            return 0
...
def load_dls_learner(folder,container_folder = 'runs',arch = resnet50,with_opt=True):
    path = Path(container_folder,folder)
    
    with open(Path(path,'dls.pickle'),'rb') as f:
        dls = pickle.load(f)
    if str(arch).startswith('<function'):
        learn = vision_learner(dls,arch,metrics=error_rate) #might be needed for models that have 256px size image inputs
        learn.path = path
        learn = learn.load('learn') #with_opt needs to be set to true for 
    else:
        if isinstance(arch,str):
            arch = timm.create_model(arch, pretrained=True, num_classes=2)
...
def get_train_test_preds(folder_str, container_folder='runs',arch=resnet50):
    plt = platform.system()
    if plt == 'Linux': pathlib.WindowsPath = pathlib.PosixPath
    if isinstance(arch,str):
        arch = timm.create_model(arch, pretrained=False, num_classes=2)
...
def get_train_test_ho_preds(folder_str, master,container_folder='runs',arch=resnet50,redo=True):
    plt = platform.system()
    if plt == 'Linux': pathlib.WindowsPath = pathlib.PosixPath
    # if isinstance(arch,str):
    #     arch = timm.create_model(arch, pretrained=True, num_classes=2)
    ...
    if os.path.exists(os.path.join(container_folder, folder_str,'preds','HO_raw', 'results.csv')):
        ...
        get_learn_preds_wrapper(ho,folder_str,os.path.join(folder_str,'preds','HO_raw'),img_path = 'Paintings/Processed/Raw',overwrite=True,folders = folders,save_pre_path = container_folder,redo=redo)
    ...
    if os.path.exists(os.path.join(container_folder, folder_str, 'preds.pickle')):
        with open(os.path.join(container_folder, folder_str, 'preds.pickle'), 'rb') as f:
             preds_valid = pickle.load(f) 
    else: #this really should never happen because preds.pickle is made when the model is first run
        preds_valid = learn.get_preds()
        with open(Path(container_folder,folder_str,'preds_valid.pickle'), 'wb') as f:
            pickle.dump(preds_valid, f)
...
# def get_train_test_ho_preds(folder_str, master,container_folder='runs',redo = True):
#     plt = platform.system()
#     if plt == 'Linux': pathlib.WindowsPath = pathlib.PosixPath
#     path = Path(container_folder,folder_str)
    
#     with open(Path(path,'dls.pickle'),'rb') as f:
#         dls = pickle.load(f)

#     learn = load_learner(Path(container_folder,folder_str,'export.pkl'))
...
#         # ho_results = pd.read_csv(os.path.join(container_folder, folder_str,'preds','HO_raw', 'results.csv'))
#     else:
#         get_learn_preds_wrapper(ho,learn,os.path.join(folder_str,'preds','HO_raw'),img_path = 'Paintings/Processed/Raw',overwrite=True,folders = folders,save_pre_path = container_folder,redo=redo)
...
def find_dist_of_pollocks_imitations(valid_list, master_df, TRAIN=False,groups = ['P', 'J']):
    if TRAIN:
    ...
    tmp = tmp.groupby('Group').agg({'numSlices': np.sum, 'file': np.count_nonzero}).reset_index()  # reset_index makes 'Group' a column rather than a row
    
    seriesI = tmp[~tmp['Group'].isin(groups)][['numSlices', 'file']].sum()
    seriesP = tmp[tmp['Group'].isin(groups)][['numSlices', 'file']].sum()
...
def extract_numerical(filename,split1 = 'D',split2 = 'pt',split3 = '-'):
    # Extracting the number before the decimal point (N1)
    number_before_decimal = filename.split(split1)[1].split(split2)[0]
    
    # Extracting the number after the decimal point (N2)
    number_after_decimal = filename.split(split2)[1].split(split3)[0]
    
    return float(number_before_decimal + '.' +number_after_decimal)